...
# plt.savefig("results/logreg/kernel/poly/withoutstd/ds2"+weight_init[wt_init])
np.savetxt("results/logreg/kernel/poly/bestlamda", configs)
if default==0 :
    plt.savefig("results/logreg/kernel/poly/"+"/ds"+dataset+"accuracy")
...
for lamda in lamdas :
    ...
    results.append(acc)
    getConfusion(y_test, pred, "logreg/kernel/poly/"+word+"/ds"+dataset+"cfmatrix"+str(count)+str(degree),"Dataset "+dataset+" for "+r"$\lambda=$"+str(lamda)+",degree="+str(degree))
    np.savetxt("results/logreg/kernel/poly/"+word+"/ds"+dataset+"traintest.txt",results,fmt="%.2f")
    count+=1
    print("Count ",count)
    boundary_plot = 1
    ...
    if boundary_plot :
        ...
        plt.xlabel("x1")
        plt.ylabel("y1")
        plt.title("Decision boundary for Dataset "+dataset+" for "+r"$\lambda=$"+str(lamda)+",degree="+str(degree))

        plt.savefig("results/logreg/kernel/poly/"+word+"/ds"+dataset+"boundary"+str(count-1)+str(degree))
